...
class SequentialEncoder(Encoder):
    ...
    def forward(self, tensor_dict: Dict[str, torch.Tensor]):
        ...
        variable_encoding = variable_encoding / (
            variable_mention_num + (1.0 - variable_encoding_mask) * nn_util.SMALL_NUMBER
        ).unsqueeze(-1)
    ...
    def encode_sequence(self, code_sequence):
        ...
        (
            ...
            restoration_indices,
            sorting_indices,
        ) = nn_util.sort_batch_by_length(code_token_embedding, code_sequence_length)

        packed_question_embedding = pack_padded_sequence(
            sorted_seqs, sorted_seq_lens.data.tolist(), batch_first=True
        )

        sorted_encodings, (last_states, last_cells) = self.lstm_encoder(
            packed_question_embedding
        )
        ...
        restored_encodings = sorted_encodings.index_select(
            dim=0, index=restoration_indices
        )
    ...
    def to_tensor_dict(cls, examples: List[Example]) -> Dict[str, torch.Tensor]:
        ...
        variable_mention_num = torch.zeros(
            len(examples), max(len(e.ast.variables) for e in examples)
        )
        ...
        for e_id, example in enumerate(examples):
            ...
            for i, sub_token in enumerate(sub_tokens):
                if sub_token.startswith("@@") and sub_token.endswith("@@"):
                    ...
                    if (
                        old_var_name in var_name_to_id
                    ):  # sometimes there are strings like `@@@@`
            ...
            variable_encoding_mask[e_id, : len(example.ast.variables)] = 1.0

        return dict(
            src_code_tokens=torch.from_numpy(input),
