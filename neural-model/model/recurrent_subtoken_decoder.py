...
class RecurrentSubtokenDecoder(Decoder):
    ...
    def forward(self, src_ast_encoding, prediction_target):
        ...
        target_variable_encoding_indices = prediction_target[
            "target_variable_encoding_indices"
        ]
        target_variable_encoding_indices_mask = prediction_target[
        ...
        variable_encoding = torch.gather(
            src_ast_encoding["variable_encoding"],
            1,
            target_variable_encoding_indices.unsqueeze(-1).expand(
                -1, -1, variable_encoding_size
            ),
        )
        # (batch_size, max_time_step, encoding_size)
        ...
        att_tm1 = variable_encoding.new_zeros(
            src_ast_encoding["batch_size"], self.lstm_cell.hidden_size
        )
        ...
        for t, variable_encoding_t in enumerate(
            variable_encoding.split(split_size=1, dim=1)
        ):
            ...
            if self.independent_prediction_for_each_variable and t < max_time_step - 1:
                ...
                is_tp1_same_variable = (
                    torch.eq(variable_ids_tp1, variable_ids_t).float().unsqueeze(-1)
                )  # TODO: check if correct!
                h_tm1 = (
                    h_tm1[0] * is_tp1_same_variable,
                    h_tm1[1] * is_tp1_same_variable,
                )
        ...
        var_name_log_probs = (
            var_name_log_probs * target_variable_encoding_indices_mask.unsqueeze(-1)
        )
            ...
            for beam_id, (ast_id, beam) in enumerate(beams.items()):
                ...
                for i in range(cont_beam_size):
                    ...
                    if hyp_var_name_id == end_of_variable_id:
                        ...
                        # remove empty cases
                        if (
                            len(prev_hyp.variable_list) == 0
                            or prev_hyp.variable_list[-1] == end_of_variable_id
                        ):
                            continue
