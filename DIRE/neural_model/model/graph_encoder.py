...
class GraphASTEncoder(Encoder):
    ...
    def forward(self, tensor_dict: Dict[str, torch.Tensor]):
        ...
        variable_encoding = variable_encoding[tensor_dict['variable_encoding_restoration_indices']] * tensor_dict['variable_encoding_restoration_indices_mask'].unsqueeze(-1)
    ...
    def get_decoder_init_state(self, context_encoding, config=None):
        ...
        node_encoding_sum = zero_encoding.scatter_add_(0, context_encoding['tree_node_to_tree_id_map'].unsqueeze(
                                                           -1).expand(-1, encoding_size),
                                                       packed_tree_node_encoding)
        tree_node_num = packed_tree_node_encoding.new_zeros(tree_num).scatter_add_(0, context_encoding['tree_node_to_tree_id_map'],
    ...
    def get_attention_memory(self, context_encoding, att_target='terminal_nodes'):
        if att_target == 'ast_nodes':
            memory = context_encoding['packed_tree_node_encoding'][context_encoding['tree_restoration_indices']]
            mask = context_encoding['tree_restoration_indices_mask']
        elif att_target == 'terminal_nodes':
            memory = context_encoding['packed_tree_node_encoding'][context_encoding['terminal_node_restoration_indices']]
