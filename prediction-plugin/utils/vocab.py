...
class VocabEntry:
    def __init__(self, subtoken_model_path=None):
        ...
        if subtoken_model_path:
            ...
            subtoken_model = subtoken_model_path[: subtoken_model_path.rfind(".model")]
    ...
    def from_corpus(corpus, size, freq_cutoff=0, predefined_tokens=None):
        ...
        print(
            f"number of word types: {len(word_freq)}, "
...
class Vocab(object):
    ...
    def __repr__(self):
        words = ", ".join(
            f"{entry} {getattr(self, entry)}words" for entry in self.entries
        )
...
if __name__ == "__main__":
    ...
    if args["--use-bpe"]:
        ...
        src_code_tokens_vocab_entry = VocabEntry(f"{vocab_file}.src_code_tokens.model")
