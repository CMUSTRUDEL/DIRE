...
from utils.dataset import Dataset, Example, get_json_iterator_from_tar_file
...
def evaluate_debin(debin_output_path, test_example_path):
    ...
    for example in debin_predicted_data.get_iterator(num_workers=5):
        example_key = (
            example.binary_file["file_name"].split("/")[-1]
            + "_"
            + str(example.ast.compilation_unit)
        )
    ...
    for example in test_set.get_iterator(num_workers=5):
        ...
        if example_key in debin_prediction_dict:
            ...
            rename_result = {
                k: k for k in example.variable_name_map
            }  # use identity predictions
...
if __name__ == "__main__":
    evaluate_debin(
        "data/debin.predictions.0.01.tar", "data/all_trees_tokenized_0410/test.tar"
    )
