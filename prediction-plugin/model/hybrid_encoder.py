...
class HybridEncoder(nn.Module):
    ...
    def forward(self, tensor_dict: Dict[str, torch.Tensor]):
        ...
        if self.hybrid_method == "linear_proj":
            ...
            variable_encoding = torch.cat(
                [graph_var_encoding, seq_var_encoding], dim=-1
            )
    ...
    def get_decoder_init_state(self, context_encoding, config=None):
        ...
        seq_dec_init_state, seq_dec_init_cell = self.seq_encoder.get_decoder_init_state(
    ...
    def get_attention_memory(self, context_encoding, att_target="terminal_nodes"):
        ...
        seq_memory, seq_mask = self.seq_encoder.get_attention_memory(
            context_encoding["seq_encoding_result"], att_target="terminal_nodes"
        )
        gnn_memory, gnn_mask = self.graph_encoder.get_attention_memory(
            context_encoding["graph_encoding_result"], att_target="terminal_nodes"
        )
