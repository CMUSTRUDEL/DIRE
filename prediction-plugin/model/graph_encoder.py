...
class GraphASTEncoder(Encoder):
    ...
    def __init__(
        ...
        self.var_node_name_embedding = nn.Embedding(
            len(vocab.source), gnn.hidden_size, padding_idx=0
        )
        ...
        if self.init_with_seq_encoding:
            ...
            if config["seq_encoder"]["source_encoding_size"] != gnn.hidden_size:
                self.seq_variable_encoding_to_graph_linear = nn.Linear(
                    config["seq_encoder"]["source_encoding_size"], gnn.hidden_size
                )
            else:
    ...
    def build(cls, config, logging=False):
        ...
        node_type_embedder = NodeTypeEmbedder(
            len(vocab.grammar.variable_types), params["node_type_embedding_size"]
        )
        node_content_embedder = SubTokenEmbedder(
            vocab.obj_name.subtoken_model_path, params["node_content_embedding_size"]
        )
    ...
    def forward(self, tensor_dict: Dict[str, torch.Tensor]):
        ...
        if self.init_with_seq_encoding:
            ...
            tree_node_init_encoding[
                ...
                seq_var_encoding.view(-1, seq_var_encoding.size(-1))[
        ...
        if "variable_master_nodes" in connections:
            ...
            variable_encoding = torch.zeros(
                variable_num, var_nodes_encoding.size(-1), device=self.device
            )
            var_nodes_encoding_sum = variable_encoding.scatter_add_(
                0,
                tensor_dict["variable_node_variable_ids"]
                .unsqueeze(-1)
                .expand_as(var_nodes_encoding),
                var_nodes_encoding,
            )
            variable_encoding = var_nodes_encoding_sum / tensor_dict[
                "variable_mention_nums"
            ].unsqueeze(-1)

        # restore variable encoding
        variable_encoding = variable_encoding[
            tensor_dict["variable_encoding_restoration_indices"]
        ] * tensor_dict["variable_encoding_restoration_indices_mask"].unsqueeze(-1)
    ...
    def to_packed_graph(
        ...
        variable_repr_restoration_indices = torch.zeros(
            len(asts), max_variable_num, dtype=torch.long
        )
        variable_repr_restoration_indices_mask = torch.zeros(
            len(asts), max_variable_num
        )
        ...
        for ast_id, ast in enumerate(asts):
            ...
            if "master_node" in connections:
                master_node_id = packed_graph.register_node(
                    ast_id, "master_node", group="master_nodes"
                )
            ...
            for i, (var_name, var_nodes) in enumerate(ast.variables.items()):
                ...
                if "var_usage" in connections:
                    for _idx in range(len(var_nodes) - 1):
                        ...
                        var_usage_adj_list.append(
                            (
                                packed_graph.get_packed_node_id(ast_id, _var_node),
                                packed_graph.get_packed_node_id(ast_id, _next_var_node),
                            )
                        )

                if "func_root_to_arg" in connections and var_nodes[0].is_arg:
                    func_root_node_id = packed_graph.get_packed_node_id(
                        ast_id, ast.root
                    )
                ...
                if use_variable_master_node:
                    var_master_node_id, node_id_in_group = packed_graph.register_node(
                        ast_id,
                        var_name,
            ...
            variable_repr_restoration_indices_mask[ast_id, : len(ast.variables)] = 1.0
        ...
        tree_restoration_indices = torch.zeros(
            packed_graph.tree_num, max_tree_node_num, dtype=torch.long
        )
        tree_restoration_indices_mask = torch.zeros(
            packed_graph.tree_num, max_tree_node_num, dtype=torch.float
        )

        max_terminal_node_num = max(
            len(tree.terminal_nodes) for tree in packed_graph.trees
        )
        terminal_node_restoration_indices = torch.zeros(
            packed_graph.tree_num, max_terminal_node_num, dtype=torch.long
        )
        terminal_node_restoration_indices_mask = torch.zeros(
            packed_graph.tree_num, max_terminal_node_num, dtype=torch.float
        )

        for ast_id in range(packed_graph.tree_num):
            packed_node_ids = list(
                packed_graph.node_groups[ast_id]["ast_nodes"].values()
            )
            tree_restoration_indices[ast_id, : len(packed_node_ids)] = torch.tensor(
                packed_node_ids
            )
            tree_restoration_indices_mask[ast_id, : len(packed_node_ids)] = 1.0
        ...
        if init_with_seq_encoding:
            tensor_dict["var_repr_flattened_positions"] = torch.tensor(
                var_repr_flattened_positions, dtype=torch.long
            )

        if use_variable_master_node:
            tensor_dict["variable_master_node_ids"] = [
                _id
                for node, _id in packed_graph.get_nodes_by_group(
                    "variable_master_nodes"
                )
            ]
        else:
            tensor_dict["variable_node_ids"] = torch.tensor(var_node_ids)
            tensor_dict["variable_node_variable_ids"] = torch.tensor(
                var_node_variable_ids
            )
            tensor_dict["variable_mention_nums"] = torch.tensor(
                var_mention_nums, dtype=torch.float
            )
    ...
    @classmethod
    def to_tensor_dict(
        cls, packed_graph: PackedGraph, grammar: Grammar, vocab: Vocab
        ...
        if node_with_subtokens_indices:
            ...
            sub_tokens_indices = np.zeros(
                (len(sub_tokens_list), max_subtoken_num), dtype=np.int64
            )
        ...
        node_type_indices = np.zeros(
            (len(node_type_tokens_list), max_typetoken_num), dtype=np.int64
        )
    ...
    def get_batched_tree_init_encoding(
        ...
        ) * torch.ne(tensor_dict["var_node_name_indices"], 0.0).float().unsqueeze(-1)

        if tensor_dict["node_with_subtokens_indices"].size(0) > 0:
            obj_node_content_embedding = self.node_content_embedder(
                tensor_dict["sub_tokens_indices"]
            )
            node_content_embedding = node_content_embedding.scatter(
                0,
                tensor_dict["node_with_subtokens_indices"]
                .unsqueeze(-1)
                .expand_as(obj_node_content_embedding),
    ...
    def unpack_encoding(
        ...
        batch_tree_node_masks = torch.zeros(
            batch_size, max_node_num, device=self.device
        )
        for e_id, syntax_tree in enumerate(batch_syntax_trees):
            example_nodes_with_batch_id = [
                ...
                for (
                    _e_id,
                    example_node_id,
                ), batch_node_id in example_node2batch_node_map.items()
                if _e_id == e_id
            ]
            sorted_example_nodes_with_batch_id = sorted(
                example_nodes_with_batch_id, key=itemgetter(0)
            )
            ...
            batch_tree_node_masks[e_id, : len(example_nodes_batch_id)] = 1.0
        ...
        batch_node_encoding.data.masked_fill_(
            (1.0 - batch_tree_node_masks).byte().unsqueeze(-1), 0.0
        )

        return dict(
            batch_tree_node_encoding=batch_node_encoding,
    ...
    def get_attention_memory(self, context_encoding, att_target="terminal_nodes"):
        packed_tree_node_enc = context_encoding["packed_tree_node_encoding"]
        if att_target == "ast_nodes":
            memory = packed_tree_node_enc[context_encoding["tree_restoration_indices"]]
