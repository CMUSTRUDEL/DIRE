...
class SimpleDecoder(Decoder):
    ...
    def default_params(cls):
        return {"vocab_file": None, "ast_node_encoding_size": 128}
    ...
    def forward(
        self,
        src_ast_encoding: Dict[str, torch.Tensor],
        ...
        # (batch_var_node_num)
        packed_tgt_var_node_name_log_probs = torch.gather(
            batched_p_names,
            dim=-1,
            index=prediction_target["variable_tgt_name_id"].unsqueeze(-1),
        ).squeeze(-1)
        ...
        with torch.no_grad():
            ...
            renamed_var_avg_ll = (
                packed_tgt_var_node_name_log_probs * renamed_var_mask
            ).sum(-1) / renamed_var_mask.sum()
            unchanged_var_avg_ll = (
                packed_tgt_var_node_name_log_probs * unchanged_var_mask
            ).sum(-1) / unchanged_var_mask.sum()
            result["rename_ppl"] = torch.exp(-renamed_var_avg_ll).item()
            result["unchange_ppl"] = torch.exp(-unchanged_var_avg_ll).item()

        packed_tgt_var_node_name_log_probs = (
            packed_tgt_var_node_name_log_probs
            * prediction_target["variable_tgt_name_weight"]
        )
        ...
        # (batch_size)
        ast_log_probs = tgt_name_log_probs.sum(dim=-1) / src_ast_encoding[
