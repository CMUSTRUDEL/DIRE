...
class GatedGraphNeuralNetwork(nn.Module):
    def __init__(
        ...
        # Prepare linear transformations from node states to messages, for each
        # layer and each edge type.
        ...
        for layer_idx in range(len(self.layer_timesteps)):
            ...
            for edge_type_j in range(self.num_edge_types):
                state_to_msg_linear_layer_i_type_j = nn.Linear(
                    self.hidden_size, self.hidden_size, bias=use_bias_for_message_linear
                )
                layer_name = (
                    f"state_to_message_linear_layer{layer_idx}" f"_type{edge_type_j}"
                )
    ...
    def forward(
        ...
        return self.compute_node_representations(
    ...
    def compute_node_representations(
        ...
        """
        # If the dimension of initial node embedding is smaller, then perform
        # padding first
        ...
        if init_node_repr_size < self.hidden_size:
            ...
            zero_pads = torch.zeros(
                node_num, pad_size, dtype=torch.float, device=self.device
            )
            initial_node_representation = torch.cat(
                [initial_node_representation, zero_pads], dim=-1
            )

        # Shape: [V, D]
        node_states_per_layer = [initial_node_representation]

        # list of tensors of message targets of shape [E]
        message_targets = []
        ...
        for layer_idx, num_timesteps in enumerate(self.layer_timesteps):
            ...
            layer_residual_connections = self.residual_connections.get(layer_idx, [])
            # List[(V, D)]
            layer_residual_states: List[torch.Tensor] = [
                node_states_per_layer[residual_layer_idx]
                for residual_layer_idx in layer_residual_connections
            ]

            # Record new states for this layer. Initialised to last state, but
            ...
            for t in range(num_timesteps):
                ...
                message_source_states: List[torch.Tensor] = []
                ...
                for edge_type_idx, adjacency_list_for_edge_type in enumerate(
                    ...
                    if adjacency_list_for_edge_type.edge_num > 0:
                        ...
                        layer_name = (
                            f"state_to_message_linear_layer{layer_idx}"
                            f"_type{edge_type_idx}"
                        )
                        f_state_to_message = self.state_to_message_linears[layer_name]

                        # Shape [E, D]
                        all_messages_for_edge_type = self.state_to_message_dropout_layer(
                ...
                incoming_messages = torch.zeros(
                    node_num, messages.size(1), device=self.device
                )
                incoming_messages = incoming_messages.scatter_add_(
                    0, message_targets.unsqueeze(-1).expand_as(messages), messages
                )
                node_target_num = torch.zeros(node_num, device=self.device).scatter_add(
                    0, message_targets, torch.ones(messages.size(0), device=self.device)
                )
                incoming_messages = incoming_messages / (
                    node_target_num + 1e-7
                ).unsqueeze(-1)
                # shape [V, D * (1 + num of residual connections)]
                incoming_information = torch.cat(
                    layer_residual_states + [incoming_messages], dim=-1
                )

                # pass updated vertex features into RNN cell
                # Shape [V, D]
                updated_node_states = self.rnn_cells[layer_idx](
                    incoming_information, node_states_for_this_layer
                )
...
def main():
    ...
    adj_list_type1 = AdjacencyList(
        node_num=4, adj_list=[(0, 2), (2, 1), (1, 3)], device=gnn.device
    )
    adj_list_type2 = AdjacencyList(
        node_num=4, adj_list=[(0, 0), (0, 1)], device=gnn.device
    )

    node_representations = gnn.compute_node_representations(
        initial_node_representation=torch.randn(4, 64),
